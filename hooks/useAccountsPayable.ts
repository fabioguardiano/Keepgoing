...
const map = (r: any): AccountPayable => ({
  ...
  accountPlanId: r.account_plan_id,
  accountPlanName: r.account_plan_name,
  dueDate: r.due_date,
...
export const useAccountsPayable = (companyId?: string, logActivity?: LogFn) => {
  ...
  const handleSavePayable = async (ap: Omit<AccountPayable, 'remainingValue'> & { id?: string }) => {
    ...
    const payload: any = {
      ...
      description: up(ap.description),
      supplier_id: ap.supplierId || null,
      supplier_name: up(ap.supplierName) || '',
      ...
      account_plan_id: ap.accountPlanId || null,
      account_plan_name: ap.accountPlanName || null,
      due_date: ap.dueDate,
