...
export const BrandsView: React.FC<BrandsViewProps> = ({ brands, onSaveBrand, onDeleteBrand, onSyncCloud }) => {
  ...
  const filteredBrands = useMemo(() => brands.filter(b => {
    ...
    const matchesStatus = showInactive ? b.status === 'inativo' : (b.status === 'ativo' || !b.status);
  ...
  }), [brands, searchQuery, showInactive, sortField, sortDirection]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="management-title">Marcas</h1>
          <p className="management-subtitle">Controle técnico de fabricantes e marcas parceiras</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-white/5">
              {filteredBrands.map((brand) => (
                <tr key={brand.id} className={`hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group ${brand.status === 'inativo' ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-black text-slate-800 dark:text-white">{brand.description}</span>
                      {brand.status === 'inativo' && (
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex justify-end gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        onClick={() => brand.status === 'inativo' ? onSaveBrand({ ...brand, status: 'ativo' }) : onDeleteBrand(brand.id)}
                        className={`p-2 rounded-xl transition-all border border-transparent ${brand.status === 'inativo' ? 'text-green-500 hover:bg-green-50 hover:border-green-100' : 'text-slate-400 hover:text-amber-600 hover:bg-amber-50 hover:border-amber-100'}`}
                        title={brand.status === 'inativo' ? 'Reativar' : 'Inativar'}
                      >
                        <PowerOff size={16} />
                      </button>
                    </div>
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center gap-2 opacity-30">
                      <Search size={48} />
                      <p className="font-bold text-slate-400">Nenhuma marca encontrada</p>
                    </div>
                  </td>
                </tr>
              )}
          ...
          <div className="management-modal rounded-[32px] w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
              <div>
                ...
                <p className="management-subtitle text-sm">Preencha os dados da marca</p>
              </div>
              <button 
