...
const CATEGORY_LABELS: Record<PaymentCategory, string> = {
  ...
  cartao_credito_prazo:  'Cartão de Crédito Parcelado',
  boleto:                'Boleto',
  cheque:                'Cheque',
  outro:                 'Outro',
};

const CATEGORY_ICON: Record<PaymentCategory, React.ReactNode> = {
  dinheiro:              <Banknote size={16} />,
  pix:                   <QrCode size={16} />,
  transferencia:         <Building2 size={16} />,
  cartao_debito:         <CreditCard size={16} />,
  cartao_credito_avista: <CreditCard size={16} />,
  cartao_credito_prazo:  <CreditCard size={16} />,
  boleto:                <FileText size={16} />,
  cheque:                <FileText size={16} />,
  outro:                 <Banknote size={16} />,
};

const CATEGORY_COLOR: Record<PaymentCategory, string> = {
  ...
  cartao_credito_avista: 'bg-orange-100 text-orange-700',
  cartao_credito_prazo:  'bg-rose-100 text-rose-700',
  boleto:                'bg-slate-100 text-slate-700',
  cheque:                'bg-yellow-100 text-yellow-700',
  outro:                 'bg-slate-100 text-slate-600',
};

const APRAZO_CATEGORIES: PaymentCategory[] = ['cartao_credito_prazo', 'boleto', 'cheque'];
...
export const PaymentMethodsView: React.FC<Props> = ({ paymentMethods, onSave, onDelete, onToggle }) => {
  ...
  const handleCategoryChange = (cat: PaymentCategory) => {
    const isAprazo = APRAZO_CATEGORIES.includes(cat);
    setForm(f => ({
      ...f,
      category: cat,
      type: isAprazo ? 'aprazo' : 'avista',
      installments: isAprazo ? (f.installments ?? 2) : 1,
    }));
  };
        ...
        <div className="divide-y divide-slate-50 dark:divide-slate-800">
          {methods.map(pm => (
            <div key={pm.id} className={`flex items-center gap-4 px-6 py-4 transition-colors ${pm.active ? '' : 'opacity-50'}`}>
              <div className={`p-2 rounded-xl ${CATEGORY_COLOR[pm.category]}`}>
                {CATEGORY_ICON[pm.category]}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-400 font-medium mt-0.5 flex items-center gap-3">
                  <span>{CATEGORY_LABELS[pm.category]}</span>
            ...
            <div className="p-6 space-y-4">
              ...
              {form.type === 'aprazo' && (
                <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-800">
                  ...
                  {(form.category === 'cartao_credito_prazo') && (
                    <>
                      <div>
