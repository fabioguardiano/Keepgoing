  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="management-title">Gestão de Fornecedores</h1>
          <p className="management-subtitle">Parceiros e fornecedores de insumos homologados</p>
        </div>
        <button
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-white/5">
              {paginatedSuppliers.map(supplier => (
                <tr key={supplier.id} className="management-row-hover group">
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 bg-slate-50 dark:bg-white/5 border-t border-slate-100 dark:border-white/5 flex items-center justify-between gap-4">
            <div className="text-sm text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap">
              Mostrando <span className="font-bold text-slate-700 dark:text-slate-200">{((currentPage - 1) * itemsPerPage) + 1}</span> a <span className="font-bold text-slate-700 dark:text-slate-200">{Math.min(currentPage * itemsPerPage, filteredAndSortedSuppliers.length)}</span> de <span className="font-bold text-slate-700 dark:text-slate-200">{filteredAndSortedSuppliers.length}</span> fornecedores
            </div>
