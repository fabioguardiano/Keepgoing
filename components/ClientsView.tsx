  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="management-title">Gestão de Clientes</h1>
          <p className="management-subtitle">Relatório técnico de base integrada</p>
        </div>
        {canEdit && (
          <button
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-white/5">
              {paginatedClients.map(client => (
                <tr key={client.id} className="management-row-hover group">
                  ...
                  <td className="px-6 py-6">
                    <div className="text-sm text-slate-700 dark:text-slate-200 font-black uppercase tracking-tight">{client.address.city}</div>
                    <div className="text-xs text-slate-400 font-bold uppercase tracking-widest">{client.address.state}</div>
                  </td>
                  <td className="px-6 py-6">
                    <div className="text-sm text-slate-600 dark:text-slate-300 font-black">
                      ...
                      {canEdit && (
                        <>
                          <button
                            ...
                            className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-all border border-transparent hover:border-blue-100 dark:hover:border-blue-900/30"
                            title="Editar"
                          >
                            <Edit2 size={16} />
                          </button>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 bg-slate-50 dark:bg-white/5 border-t border-slate-100 dark:border-white/5 flex items-center justify-between gap-4">
            <div className="text-sm text-slate-500 dark:text-slate-400 font-medium whitespace-nowrap">
              Mostrando <span className="font-bold text-slate-700 dark:text-slate-200">{((currentPage - 1) * itemsPerPage) + 1}</span> a <span className="font-bold text-slate-700 dark:text-slate-200">{Math.min(currentPage * itemsPerPage, filteredAndSortedClients.length)}</span> de <span className="font-bold text-slate-700 dark:text-slate-200">{filteredAndSortedClients.length}</span> clientes
            </div>
